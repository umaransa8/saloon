  ...
  $QUEY = "";
		...
		echo "<table class='table table-bordered' id='table'> <thead>";
				...
				if($reportfor  == "ALL") {
					if($typeselect == "in") {
						if($invoice == "ALL"){
							$QUEY = "select DISTINCT a.invoice_no,a.total_item,a.transaction_date,a.total_amount,concat(b.Name,' - ',b.MobileNumber) as Customer from saloon_sale_transaction a, tblcustomers b, saloon_sale_service_cart c where a.invoice_no = c.cart_invoice_no and  a.customer_id=b.ID ";
						}
						else {
							$QUEY = "select DISTINCT a.invoice_no,a.total_item,a.transaction_date,a.total_amount,concat(b.Name,' - ',b.MobileNumber) as Customer from saloon_sale_transaction a, tblcustomers b, saloon_sale_service_cart c where a.invoice_no = c.cart_invoice_no and  a.customer_id=b.ID  and a.invoice_no = '$invoice'";
						}
					}
					else{
						$QUEY = "select DISTINCT a.invoice_no,a.total_item,a.transaction_date,a.total_amount,concat(b.Name) as Customer from saloon_sale_transaction a, tblcustomers b, saloon_sale_service_cart c  where a.invoice_no = c.cart_invoice_no and  a.customer_id=b.ID and (date(a.transaction_date) between '$startdate' and '$enddate') ORDER BY invoice_no desc";
					}
				}
				else {
					if($typeselect == "in") {
						if($invoice == "ALL"){
							$QUEY = "select DISTINCT a.invoice_no,a.total_item,a.transaction_date,a.total_amount,concat(b.Name,' - ',b.MobileNumber) as Customer from saloon_sale_transaction a, tblcustomers b, saloon_sale_service_cart c where a.invoice_no = c.cart_invoice_no and  a.customer_id=b.ID and a.bill_type = '$reportfor'";
						}
						else {
							$QUEY = "select DISTINCT a.invoice_no,a.total_item,a.transaction_date,a.total_amount,concat(b.Name,' - ',b.MobileNumber) as Customer from saloon_sale_transaction a, tblcustomers b, saloon_sale_service_cart c where a.invoice_no = c.cart_invoice_no and  a.customer_id=b.ID  and a.invoice_no = '$invoice' and a.bill_type = '$reportfor'";
						}
					}
					else{
			...
			if($co > 0) {
				...
				echo "<tr> 
						...
						<td> <a href='editinvoice.php?invoice_no=".$row['invoice_no']."'> <input type='button' data-toggle='modal' data-target='#editmod' class='btn btn-primary edit' id='".$row['invoice_no']."' value='Edit' /></a> </td>
						<td><input type='button' class='btn btn-danger reject delete ' id='".$row['invoice_no']."' value='Cancel'/></td>
						</tr>";
					$i++;
			}
			}
...
if($action == "getinvoicem") {
	...
	$saloon_sale_service_cart_query = "SELECT cart_id, service_name, actual_price, discount_price, cgst, sgst, total, (SELECT CONCAT(Name , ' - ', MobileNumber ) as employee FROM saloon_employee WHERE ID = employee_id) as employee FROM saloon_sale_service_cart  WHERE cart_invoice_no = '$id'" ;
	//error_log($saloon_sale_service_cart_query);
	$saloon_sale_service_cart_result=mysqli_query($con,$saloon_sale_service_cart_query );
	echo "<div class='modal-body cusmodel' style='padding:0px'> 
			<div class='col-lg-8'>
