...
$id = trim($_POST['id']);
 if($action == "query") {	
	...
	$edate = $_POST['enddate'];
	$QUEY = "SELECT invoice_no FROM saloon_sale_transaction WHERE date(transaction_date) between '$sdate' and  '$edate' and bill_type = 'C'";
	error_log($QUEY);
	...
	if($ret) {
		...
		if($co > 0) {	
			...
			echo "<tr> 
						<td style='text-align:center;width:25%' class='eve'>".$co."</td> 
						<td style='text-align:center;width:15%' class='eve'><input type='button' class='btn btn-primary' id='Print' value='Print All'/></td> ";
			
		}
		else {
			echo "<tr><th colspan='2'>No Data Found..</th> </tr>";
		}
	}
	else {
		echo "<tr><th colspan='2'>".mysqli_error($con)."</th> </tr>";
	}
	echo "</tbody></table>";
 ...
 if($action == "print") {	
	...
	$query1 = "SELECT invoice_no FROM saloon_sale_transaction WHERE date(transaction_date) between '$sdate' and  '$edate' and bill_type='C'";
	error_log($query1);
	$result1 = mysqli_query($con,$query1 );
	$cou = mysqli_num_rows($result1);
	echo $cou."BRK";
	if($cou > 0) {
		while($check_row = mysqli_fetch_assoc($result1)) {
			$id = $check_row['invoice_no'];
